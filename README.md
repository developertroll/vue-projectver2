...
```
고민끝에 quill 엎는 대신 데이터를 html로 처리해서 읽고 쓰기 쉽게 바꿀 예정. ReplyGroup 만드는 과정에서 userAvatar 관련해서 계속해서 오류가 발생해 내일 완성해야할듯.
```

### 20231220 일지

```
자고 일어나서 보니 그냥 original을 읽게 해서 번역되지 않은 값을 들어가게 하면 됬음. 어짜피 해당 페이지에서 해당 값들이 들어가지 않아서 번역값을 보여줄 필요가 없었음. 카톡 그룹방 느낌으로 Talk 제작 완료. 메인페이지에 무엇이 들어갈지만 생각해서 제작하면 build해서 깃허브에 페이지로 올릴 예정.
```

### 20231222 일지

```
메인 페이지는 이 그룹웨어에 대한 간략한 설명과 목적을 작성하고, 특정 페이지로 이동하는 버튼들을 만들기로 함.
그리고 결재 관련해서 지금 보니 현재 로그인한 멤버를 체크하는 함수가 없었어서 누가 요청하던 결재가 진행되던 것을 확인. 수정함.
```
